...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    next_h = np.tanh(np.dot(xh, Wxh) + b)
...
def rnn_step_backward(dnext_h, cache):
    ...
    dout = dnext_h * (1 - next_h ** 2)  # tanh'(x) = 1 - (tanh(x))^2
    dx = dout @ Wx.T
    dprev_h = dout @ Wh.T
    dWx = x.T @ dout
    dWh = prev_h.T @ dout
    ...
    # 又是一次過，寫layers就是做assignment最開心的事。
...
def rnn_backward(dh, cache):
    ...
    for i in reversed(range(T)):
      dx_i, dh_i, dWx_i, dWh_i, db_i = rnn_step_backward(dh[:, i, :] + dh_i, cache.pop())
      dx[:, i, :] = dx_i
      dWx += dWx_i
      dWh += dWh_i
      db += db_i
...
def word_embedding_forward(x, W):
    ...
    # 又是一次過，都還沒學cs224n也沒學word embedding呢，寫layers真開心。
...
def word_embedding_backward(dout, cache):
    ...
    for v in range(V):
      dW[v, :] += dout[x == v, :].sum(axis=0)

    # 驚了，這個也能一次過，這裡說的一次過都是改掉語法錯誤之後的邏輯無誤一次過，現在維度遊戲玩得不錯，只是這些數字在腦子裡仍然抽象
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    xh = np.concatenate((x, prev_h), axis=1)
    Wxh = np.concatenate((Wx, Wh), axis=0)
    inputs = np.dot(xh, Wxh) + b  # (N, 4H)
    i = sigmoid(inputs[:, :H])
    f = sigmoid(inputs[:, H:2*H])
    o = sigmoid(inputs[:, 2*H:3*H])
    g = np.tanh(inputs[:, 3*H:4*H])
    ig = i * g
    next_c = f * prev_c + ig  # (N, H)
    tanhc = np.tanh(next_c)
    next_h = o * tanhc

    cache = tanhc, i, f, o, g, prev_c, prev_h, x, Wxh

    # 一次過，LSTM實現還是很容易的，就是比較抽象
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    tanhc, i, f, o, g, prev_c, prev_h, x, Wxh = cache
    
    D, H = x.shape[1], prev_h.shape[1]

    do = dnext_h * tanhc * o * (1 - o)
    dnext_c = dnext_c + dnext_h * o * (1 - tanhc ** 2)
    dprev_c = dnext_c * f
    ...
    dxh = np.dot(difog, Wxh.T)  # (N, D+H)
    dx = dxh[:, :D]
    dprev_h = dxh[:, D:D+H]
    
    db = difog.sum(axis=0)

    # 別漏了ifog的activation function
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    (N, T, D), H = x.shape, h0.shape[1]
    prev_h = h0
    h, c, caches = np.zeros((N, T, H)), np.zeros((N, H)), []

    for i in range(T):
      prev_h, c, cache = lstm_step_forward(x[:,i,:], prev_h, c, Wx, Wh, b)
      h[:, i, :] = prev_h
      caches.append(cache)
...
def lstm_backward(dh, cache):
    ...
    (N, T, H), D = dh.shape, cache[0][7].shape[1]
    ...
    # dnext_c = dnext_c + dnext_h * o * (1 - tanhc ** 2)
    # dc = dh[:, T-1, :] * cache[T-1][3] * (1 - cache[T-1][0] ** 2)
    dc = np.zeros((N, H))  # 草，你媽想半天沒想明白第一個梯度值是0，不是1也不是用dh初始化，說白了雖然vanilla裡dh[:, i, :] + dh_i自己蒙對了，但其實沒搞明白hidden state梯度尤其是第一個怎麼傳的
    for i in reversed(range(T)):
      dx_i, dh_i, dc, dWx_i, dWh_i, db_i = lstm_step_backward(dh[:, i, :] + dh_i, dc, cache.pop())
      dx[:, i, :] = dx_i
      dWx += dWx_i
      dWh += dWh_i
      db += db_i
...
def temporal_affine_forward(x, w, b):
    ...
    N, T, D = x.shape
    M = b.shape[0]
    out = x.reshape(N * T, D).dot(w).reshape(N, T, M) + b  # 只用同一個 FC
    cache = x, w, b, out
    return out, cache
