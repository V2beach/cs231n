...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        captions = self.positional_encoding(captions)  # (N, max_length, wordvec_dim) = (N, T, W)
        # features = self.visual_projection(features).unsqueeze(1).expand(N, T, captions.shape[2])  # (N, wordvec_dim) -> (N, max_length, wordvec_dim)
        features = self.visual_projection(features).unsqueeze(1)  # (N, wordvec_dim) -> (N, max_length, wordvec_dim)

        # 經測試mask在他這個例子裡沒用
        tgt_mask = torch.flip(torch.tril(torch.ones(T, T, dtype=torch.bool)), dims=[1])  # torch.int的int32不行，只能用bool
        # tgt_mask = ~torch.flip(torch.tril(torch.ones(T, T, dtype=torch.bool), diagonal=-1), dims=[1])
        # tgt_mask = torch.tril(torch.ones(T, T, dtype=torch.bool))
        # tgt_mask = ~torch.tril(torch.ones(T, T, dtype=torch.bool), diagonal=-1)
        # tgt_mask = None
        # print(tgt_mask)
